...
class ApplicationProxyRuleArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[Sequence[pulumi.Input[str]]] origin_values: Origin server information.When `OriginType` is custom, this field value indicates multiple origin servers in either of
        ...
        :param pulumi.Input[str] forward_client_ip: Passes the client IP. Default value is OFF.When Proto is TCP, valid values:- `TOA`: Pass the client IP via TOA.- `PPV1`:
        ...
        :param pulumi.Input[str] status: Status of this application proxy rule. Valid values to set is `online` and `offline`.- `online`: Enable.- `offline`:
