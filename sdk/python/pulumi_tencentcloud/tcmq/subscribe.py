...
class SubscribeArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] endpoint: `Endpoint` for notification receipt, which is distinguished by `Protocol`. For `http`, `Endpoint` must begin with
        ...
        :param pulumi.Input[str] protocol: ubscription protocol. Currently, two protocols are supported: `http` and `queue`. To use the `http` protocol, you need
        ...
        :param pulumi.Input[str] subscription_name: Subscription name, which must be unique in the same topic under the same account in the same region. It can contain up
               to 64 letters, digits, and hyphens and must begin with a letter.
        :param pulumi.Input[str] topic_name: Topic name, which must be unique in the same topic under the same account in the same region. It can contain up to 64
               letters, digits, and hyphens and must begin with a letter.
        :param pulumi.Input[Sequence[pulumi.Input[str]]] binding_keys: The number of `BindingKey` cannot exceed 5, and the length of each `BindingKey` cannot exceed 64 bytes. This field
        ...
        :param pulumi.Input[Sequence[pulumi.Input[str]]] filter_tags: Message body tag (used for message filtering). The number of tags cannot exceed 5, and each tag can contain up to 16
               ...
               subscriber.
        :param pulumi.Input[str] notify_content_format: Push content format. Valid values: 1. JSON; 2. SIMPLIFIED, i.e., the raw format. If `Protocol` is `queue`, this value
               must be `SIMPLIFIED`. If `Protocol` is `http`, both options are acceptable, and the default value is `JSON`.
        :param pulumi.Input[str] notify_strategy: CMQ push server retry policy in case an error occurs while pushing a message to `Endpoint`. Valid values: 1.
...
class Subscribe(pulumi.CustomResource):
    ...
    def __init__(__self__,
                 ...
                 __props__=None):
        """
        Create a Subscribe resource with the given unique name, props, and options.
