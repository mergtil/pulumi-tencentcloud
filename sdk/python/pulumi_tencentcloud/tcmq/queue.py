...
class QueueArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] queue_name: Queue name, which must be unique under the same account in the same region. It can contain up to 64 letters, digits, and
        ...
        :param pulumi.Input[int] max_msg_heap_num: Maximum number of heaped messages. The value range is 1,000,000-10,000,000 during the beta test and can be
        ...
        :param pulumi.Input[int] max_time_to_live: Maximum period in seconds before an unconsumed message expires, which is required if `policy` is 1. Value range:
               300-43200. This value should be smaller than `msgRetentionSeconds` (maximum message retention period).
        :param pulumi.Input[int] msg_retention_seconds: The max period during which a message is retained before it is automatically acknowledged. Value range: 30-43,200
        ...
        :param pulumi.Input[int] retention_size_in_mb: Queue storage space configured for message rewind. Value range: 10,240-512,000 MB (if message rewind is enabled). The
               value `0` indicates that message rewind is not enabled.
        :param pulumi.Input[int] rewind_seconds: Rewindable time of messages in the queue. Value range: 0-1,296,000s (if message rewind is enabled). The value `0`
